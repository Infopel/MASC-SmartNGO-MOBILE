...
import { catchError, debounceTime, map, filter, startWith, switchMap, combineLatestWith, withLatestFrom } from 'rxjs/operators'
import { sync, whenUpdatableDataSetChanges } from 'storage/sync'
const TIME_DELAY_IN_MILLIS = 5000
            ...
            syncSubscription.current = whenUpdatableDataSetChanges()
                .pipe(
                    ...
                    , map
                        ...
                        (([changes, isInternetReachable, appState]) => {
                            console.log("WithlastestForm", { changes, isInternetReachable, appState })
                    ...
                    filter(x => x === true),
                    switchMap(async () => { setIsSyncing(true); return await sync() }),
                    map(() => setIsSyncing(false)),
                    catchError(async (error, caught) => { console.log("Syncronization error", error, caught); setIsSyncing(false), setErrorMessage("Could not sync to database!") })
                ).subscribe()
        }
